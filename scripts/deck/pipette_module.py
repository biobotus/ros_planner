...
class PipetteModule(DeckModule):
    ...
    def _parse_transfert(self, trans_json, module_dic):
        ...
        # Setup the coordinate for the dump and the maximum z coordinate for efficiency
        ...
        tip_size = self.get_tip_size(trans_json["volume"])
        self.get_tip(tip_size, module_dic)
        ...
        # Going to the source position for an operation
        ...
        # if the tip move over the same module, z coordinate will stay low
        if from_mod == to_mod:
        ...
        self.steps.append(self.move_pos(Coordinate(to_coord.coord_x, to_coord.coord_y, self.height), module_dic))
        tip_size = self.get_tip_size(trans_json["volume"])
        self.eject_tip(tip_size, module_dic)
        return
    ...
    def move_pos(self, coord, module_dic):
        if int(coord.coord_x) or int(coord.coord_y) or int(coord.coord_z) < 0:
    ...
    def get_tip_size(self,volume):
        ...
        if str(volume)  > 100 and str(volume) <= 600:
            size = "Large"

        elif str(volume) > 10 and str(volume) <= 100:
            size = "Medium"
    ...
    def get_tip(self, tip_size, module_dic):
        ...
        if tip_size=="Large":
            ...
            to_coord = self.actual_mod_pos(module_dic, tip_mod.get_well_coordinate(self.xL[0],self.yL[0]))
            to_coord.coord_z = to_coord.coord_z+6.51
            self.steps.append(self.move_pos(to_coord, module_dic)) # get up
    ...
    def eject_tip(self,tip_size, module_dic):
        ...
        to_coord = Coordinate(0,0,0)
        # Get trash module from deck
        trash_mod = module_dic["trash"]
        dump_coord  = self.actual_mod_pos(module_dic, trash_mod.get_mod_coordinate())
        to_coord.coord_z = dump_coord.coord_z # to move at trash's z coordinate
        to_coord.coord_y = dump_coord.coord_y+50 # to move at the middle of the trash
        to_coord.coord_x = dump_coord.coord_x+79.1 # to move just in front of the trash teeth
        self.steps.append(self.move_pos(to_coord, module_dic)) # x-y-z move, just in front of the dump teeth

        # step down according to tip size
        if tip_size=="Large":
        ...
        # ready to move away (move up)
        to_coord.coord_z = self.height
        self.steps.append(self.move_pos(to_coord, module_dic))
        # Reajust z0 position (as the tip is no more the z0 position)
        if tip_size=="Large":
